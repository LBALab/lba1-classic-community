...
IFDEF	GUS
		...
		db	'Adeline conversion by Serge Plagnol.', 0Dh, 0Ah
...
CurrentList             db      STRUCT_SIZE*LIST_SIZE dup (?)
SonList                 db      STRUCT_SIZE*LIST_SIZE dup (?)

BackCurrentList         db      STRUCT_SIZE*LIST_SIZE dup (?)
BackSonList             db      STRUCT_SIZE*LIST_SIZE dup (?)
...
ifdef   SINGLE_DMA
		call    BlockTransfert                  ; Normal ACK location
endif
...
noadjust0:
		...
		dec     word ptr[ebx+REPEAT]            ; repeat again?
...
next00:		mov	ax, [esi]			; read data
		...
		add     [edi], ebx			; add with buffer content (one bit might jump from right to left, but oh well...)
		add     edi, 4				; next location
		add     dx, bp				; update fractional part of address
		dec     cx                              ; length-1, doesn't touch C but set Z ;-)
...
start00:        lodsb                                   ; read new data
		...
		imul	ebx, edx
		mov	bx, ax
next00:         add     dword ptr[edi], ebx             ; add with buffer content (one bit might jump from right to left, but oh well...)
		add     edi, 4				; next location
		add     ecx, ebp                        ; update fractional part of address
		dec     cx                              ; length-1, doesn't touch C but set Z ;-)
		ja      short next00                    ; if (not C) and (not Z) next
...
EndSon:
		...
		jmp     FinishFill
Finish:
		mov     dword ptr[ebx], esi             ; save current address
		shr	ecx, 16
		mov     word ptr[ebx+FRACT], cx         ; save fractional part
Finish2:        add     ebx, STRUCT_SIZE                ; point to next sample
		jmp     NextSample
Reset:
		mov     esi, dword ptr[ebx+START]       ; restore source, no need to write it
		mov     ebp, dword ptr[ebx+DLENGTH]     ; restore length
...
SearchEnd:	add	edi, STRUCT_SIZE
		...
		mov     word ptr[edi+REPEAT], cx        ; Repeat
