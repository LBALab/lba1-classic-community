...
void lit_bodyILBM(UBYTE *ptr)
{
	...
	if ((bufligne = Malloc(MAXBUFLIGNE)) == 0)
		return;
...
LONG Load_Lbm(char *name, UBYTE *ptrscreen, UBYTE *ptrpal)
{
	...
	while (TRUE)
	{
		...
		if (len & 1)
			len++;
		if (strcmp(head, "BMHD") == 0)
			lit_bmhd();
		else if (strcmp(head, "CMAP") == 0) /* pal */
			Read(Handle, ptrpal, 768);
		else if (strcmp(head, "BODY") == 0)
		{
			if (type == 0)
			...
			lit_bloc(); /* bloc inconnu on le saute */
	}
